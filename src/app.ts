...
// DB ping at start (incluye URL destino safe)
(async () => {
  try {
    const runner = (prisma as unknown as { $runCommandRaw?: (cmd: unknown) => Promise<unknown> }).$runCommandRaw;
    if (runner) await runner({ ping: 1 });
    const raw = process.env.DATABASE_URL || '';
    ...
    console.error('[USERS] DB ping failed', e);
  }
})();
